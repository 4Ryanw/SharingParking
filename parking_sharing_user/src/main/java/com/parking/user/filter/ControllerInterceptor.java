package com.parking.user.filter;
...
public class ControllerInterceptor {
    ...
    public void doAfterReturing(Object obj) {
        ...
        //得到当前时间
  		...
  		//产生一个用户（门面对象）
  		User user = null;
  		if (uri.indexOf("insertuser")==-1 && uri.indexOf("error")==-1) {
        ...
        if (uri.indexOf("login")!=-1 || uri.indexOf("Login")!=-1) {
            ...
            if (user!=null && user.getUsername()!=null) {
            	bean.setL_userName(user.getUsername());
          		...
          		logService.loginLog(bean);
			}
      		
		}
        
        //操作增删改日志
        if (uri.indexOf("login")==-1 && uri.indexOf("find")==-1 && uri.indexOf("loginout")==-1 && uri.indexOf("logout")==-1 && uri.indexOf("error")==-1 && uri.indexOf("insertuser")==-1 ) {
       ...
       //请求响应日志
        if (uri.indexOf("loginout")==-1 && uri.indexOf("logout")==-1 && uri.indexOf("error")==-1) {
	        ...
	        logService.performanceLog(perBean);
        }
